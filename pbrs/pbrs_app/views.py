...
def index(request):
    recent_listings = HouseListing.objects.order_by('-updated')[:6]
...
def listings(request):
    ...
    financing = request.GET.get('financing', '')
    ...
    if financing:
        q_objects.add(Q(financing__icontains=financing), Q.AND)
    if property_type:
        q_objects.add(Q(property_type__icontains=property_type), Q.AND)
    
    if query or min_price or max_price or min_lot_area or max_lot_area or city or property_type or (financing and len(financing) > 0):
        house_listings = HouseListing.objects.filter(q_objects).distinct().order_by('-updated')
    else:
        house_listings = HouseListing.objects.all().order_by('-updated')
    ...
    financingTypes = HouseListing.objects.values_list('financing', flat=True).distinct()
